...
interface RadioGroupProps extends Omit<HTMLAttributes<HTMLDivElement>, "onSelect"> {
  ...
  value?: string;
  onValueChange?: (value: string) => void;
  /** Layout direction of the items — determines which axis proximity-hover
   *  tracks. Defaults to "vertical" (the common list-row case); pass
   *  "horizontal" for chip/segmented-control layouts (e.g. flex-row). */
  orientation?: "horizontal" | "vertical";
}

const RadioGroup = forwardRef<HTMLDivElement, RadioGroupProps>(
  ({ children, selectedIndex, value, onValueChange, orientation = "vertical", className, ...props }, ref) => {
...
const RadioItem = forwardRef<HTMLDivElement, RadioItemProps>(
  ({ label, index, selected, onSelect, value, icon: Icon, className, ...props }, ref) => {
        ...
        <div className="relative w-[15px] h-[15px] shrink-0">
          ...
          <AnimatePresence>
            {isSelected && (
              ...
              >
                <div className="w-[8px] h-[8px] rounded-full bg-[#C63663]" />
              </motion.div>
            )}
          </AnimatePresence>
        </div>
