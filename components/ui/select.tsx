...
interface SelectTriggerProps
  ...
  placeholder?: string;
  error?: string;
  /** Custom render for the selected value (e.g. a color swatch + label),
   *  in place of the plain text label resolved from SelectItem children. */
  renderValue?: (value: string) => ReactNode;
}

const SelectTrigger = forwardRef<HTMLButtonElement, SelectTriggerProps>(
  (
    { className, variant, icon: Icon, placeholder = "Select…", error, renderValue, ...props },
    ref
  ) => {
            ...
            <SelectContentContext.Provider value={contentCtx}>
              <SelectPrimitive.Popup
                ...
                className={cn(
                  ...
                  `relative flex flex-col gap-2 min-w-[var(--anchor-width)] max-h-[min(300px,var(--available-height))] overflow-y-auto ${shape.container} p-1 select-none outline-none`,
                  className
                )}
              >
                {/* Selected background */}
